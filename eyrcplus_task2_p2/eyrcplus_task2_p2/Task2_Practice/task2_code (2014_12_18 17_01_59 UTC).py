...
class EyantraHeap:
    ...
    def remove(self,index):
        ...
        self.exchange(self.size-1,index)
        self.size-=1
        #print "rem ",self.size
...
def insertInHeap(row,col,dis):
    if (row+1)<=9:
        ...
        if not((temp in brc) or track[row+1][col].in_setA):
            ...
            print row+1,col,dis,row,col
        else:
            print temp in brc,track[row+1][col].in_setA,row+1,col
    else:
        print row+1
    if (row-1)>=0:
        temp=(row-1)*10+col
    ...
    if (col+1)<=9:
        ...
        if not((temp in brc) or track[row][col+1].in_setA):
            ...
            print row,col+1,dis,row,col
        else:
            print temp in brc,track[row][col+1].in_setA,row,col+1
    else:
        print col+1
    if (col-1)>=0:
        temp=row*10+(col-1)
